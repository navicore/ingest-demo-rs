...
impl Record {
    ...
    pub fn generate_random(example: &Record, rng: &mut impl Rng) -> Self {
        ...
        let boat_num = rng.gen_range(1..=100);

        Self {
            version: example.version.clone(),
            name: format!("Boat {}", boat_num),
            uuid: format!("urn:mrn:signalk:uuid:{}", uuid::Uuid::new_v4()),
            latitude: example.latitude + rng.gen_range(-1.0..1.0),
            longitude: example.longitude + rng.gen_range(-1.0..1.0),
            altitude: rng.gen_range(-10.0..100.0),
            course: rng.gen_range(0.0..360.0),
            speed: rng.gen_range(0..25),
            timestamp,
        }
    }
